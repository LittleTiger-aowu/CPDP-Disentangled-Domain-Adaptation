...
def main() -> None:
    ...
    parser.add_argument("--include-projects", default=None, help="comma-separated list of projects to include")
    parser.add_argument("--exclude-projects", default=None, help="comma-separated list of projects to exclude")
    parser.add_argument("--target-project", type=str, default=None, help="target project name to exclude")
    ...
    if args.include_projects:
        keep = {p.strip() for p in args.include_projects.split(",") if p.strip()}
        dataset.df = dataset.df[dataset.df["project"].isin(keep)].reset_index(drop=True)
    if args.exclude_projects:
        drop = {p.strip() for p in args.exclude_projects.split(",") if p.strip()}
        dataset.df = dataset.df[~dataset.df["project"].isin(drop)].reset_index(drop=True)
    if args.target_project:
        dataset.df = dataset.df[dataset.df["project"] != args.target_project].reset_index(drop=True)

    if len(dataset) == 0:
        raise ValueError("No samples after project filtering")

    train_projects = set(dataset.df["project"].unique().tolist())
    source_projects = [
        name
        for name, pid in sorted(project_vocab.items(), key=lambda kv: kv[1])
        if name in train_projects
    ]
    source_project_vocab = {name: i for i, name in enumerate(source_projects)}
    ...
    model = RepresentationModel(
        ...
        d_sh=args.d_sh,
        d_pr=args.d_pr,
        num_projects=len(source_project_vocab),
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(iter_loader, start=1):
            ...
            proj_ids = torch.tensor(
                [source_project_vocab[p] for p in meta["project"]],
                dtype=torch.long,
                device=device,
            )
        ...
        ckpt = {
            ...
            "source_project_vocab": source_project_vocab,
            "config": vars(args),
        }
